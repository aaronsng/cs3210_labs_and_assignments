...
int goi(int nThreads, int nGenerations, const int *startWorld, int nRows, int nCols, int nInvasions, const int *invasionTimes, int **invasionPlans)
{
    ...
    // initialise an array to store the total death count. this is to ELIMINATE data contention
    int * divided_death_toll_;
    divided_death_toll_ = malloc(sizeof(int) * nThreads);
    for (int i = 0; i < nThreads; i++) 
    {
        divided_death_toll_[i] = 0;
    }
    ...
    // divide threads evenly 
    int * rowStartCollection;
    int * rowEndCollection;
    rowStartCollection = malloc(sizeof(int) * nThreads);
    rowEndCollection = malloc(sizeof(int) * nThreads);
    ...
    for (int i = 0; i < nThreads; i++) 
    {
        ...
        rowStartCollection[i] = startIndex;
        rowEndCollection[i] = endIndex;
    } 
    ...
    // Begin simulating
    int invasionIndex = 0;
    int tid, rowStart, rowEnd, rowWidth;
    int * wholeNewWorld; 

    #pragma omp parallel shared (world, divided_death_toll_) private (tid, rowEnd, rowStart, wholeNewWorld, invasionIndex)
    {
        tid = omp_get_thread_num();
        
        // Calculate rowWidth
        rowStart = rowStartCollection[tid];
        rowEnd = rowEndCollection[tid];
        printf("Thread init %d, rowStart %d, rowEnd %d\n", tid, rowStart, rowEnd);
        ...
        for (int i = 1; i <= nGenerations; i++)
        {
            ...
            if (invasionIndex < nInvasions && i == invasionTimes[invasionIndex])
            {
                ...
                for(int row = rowStart; row < rowEnd; row++) 
                {
                    for (int col = 0; col < nCols; col++) 
                    {
                        setValueAt(inv, rowEnd, nCols, row, col, getValueAt(invasionPlans[invasionIndex], rowEnd, nCols, row, col));
                    }
                }
                invasionIndex++;
            }
            ...
            wholeNewWorld = malloc(sizeof(int) * nRows * nCols);
            ...
            for (int row = rowStart; row < rowEnd; row++)
            {
                for (int col = 0; col < nCols; col++)
                {
                    ...
                    setValueAt(wholeNewWorld, rowEnd, nCols, row, col, nextState);
                    if (diedDueToFighting)
                    {
                        divided_death_toll_[tid] += 1;
                    }
                }
            }
            ...
            for (int row = rowStart; row < rowEnd; row++) 
            {
                for (int col = 0; col < nCols; col++) 
                {
                    setValueAt(world, rowEnd, nCols, row, col, getValueAt(wholeNewWorld, rowEnd, nCols, row, col));
                }
            }
            free(wholeNewWorld);
            #pragma omp barrier
