 ...
 * To test producer consumer using processes and semaphores. Customised to change certain test variables to verify speed
 * Compile: gcc -pthread -o ex9-processes ex9-prod-con-processes.c
 * Run: ./ex9-processes <BUFFER_SIZE> <PRODUCER_LIMIT> <CONSUMER_LIMIT>
 *************************************************************************************************************************/
...
{
    /*      loop variables          */
    int i, produce_limit, consume_limit, buffer_size;
    /*      shared memory key       */
    ...
    if (argc < 4) {
        ...
        printf("./ex9-processes <BUFFER_SIZE> <PRODUCE_LIMIT> <CONSUME_LIMIT>");
        exit(-1);
    }

    buffer_size = atoi(argv[1]);
    produce_limit = atoi(argv[2]);
    consume_limit = atoi(argv[3]);
    ...
    *r = -1;
    ...
    // i = 2  :  Consumer
        ...
        if (i == 0) {
            ...
            while (1) {
                if (consumed_count >= consume_limit)
                    break;
