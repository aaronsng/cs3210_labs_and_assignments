...
int main(int argc, char *argv[])
{
    ...
    if (argc < 9)
    {
        ...
        fprintf(stderr, "Usage: %s <INPUT_PATH> <OUTPUT_PATH> <GRID_X> <GRID_Y> <GRID_Z> <BLOCK_X> <BLOCK_Y> <BLOCK_Z> [<OPT_EXPORT_PATH>]\n", argv[0]);
#else
    ...
    // run the simulation
    dim3 gridDim_(atoi(argv[3]), atoi(argv[4]), atoi(argv[5]));
    dim3 blockDim_(atoi(argv[6]), atoi(argv[7]), atoi(argv[8]));
    int warDeathToll = goi(nThreads, nGenerations, startWorld, nRows, nCols, nInvasions, invasionTimes, invasionPlans, gridDim_, blockDim_);
...
int readWorldLayout(FILE *fp, char **line, size_t *len, int *world, int nRows, int nCols)
{
    for (int row = 0; row < nRows; row++)
    {
        ...
        for (int col = 0; col < nCols; col++)
        {
            ...
            setValueAtHost(world, nRows, nCols, row, col, cell);
            p = end;
        }
    }
